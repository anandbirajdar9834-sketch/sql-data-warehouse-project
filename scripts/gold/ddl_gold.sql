...
SELECT
    ...
    pc.maintaince AS maintaince,
    pn.prd_cost AS cost,
    pn.prd_line AS product_line,
    pn.prd_start_dt AS start_date
FROM dw_silver.crm_prd_info AS pn
LEFT JOIN dw_silver.erp_px_cat_g1v2 AS pc
    ON pn.cat_id = pc.id
WHERE pn.prd_end_dt IS NULL;

-- ---------------------------------------------------------------------------
-- Create Fact View: fact_sales
-- ---------------------------------------------------------------------------
...
LEFT JOIN dw_gold.dim_products AS pr
    ON sd.sls_prd_key = pr.product_number
LEFT JOIN dw_gold.dim_customers AS cu
